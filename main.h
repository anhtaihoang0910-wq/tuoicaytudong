...
int main(void)
{
  ...
  MX_I2C1_Init();
  MX_TIM2_Init();
 CLCD_I2C_Init(&LCD1, &hi2c1, 0x4e , 20, 4);  // n?u kh�ng hi?n th?, th? 0x27
 HAL_Delay(500);
  CLCD_I2C_Clear(&LCD1);
  // ===== Hien thi thong tin khoi dong
  CLCD_I2C_SetCursor(&LCD1, 0, 0);
  CLCD_I2C_WriteString(&LCD1, "Nhom 18 TNT");
  HAL_Delay(500);
CLCD_I2C_Clear(&LCD1);
CLCD_I2C_SetCursor(&LCD1, 0, 0);
CLCD_I2C_WriteString(&LCD1, "Temp:      C");  // ch?a tr?ng gi?a Temp: v� C

CLCD_I2C_SetCursor(&LCD1, 0, 1);
CLCD_I2C_WriteString(&LCD1, "Humi:      %");  // ch?a tr?ng gi?a Humi: v� %
...
HAL_Delay(500);
  ...
  DHT11_handle(); // d?c d? li?u
    ...
    char buf[8]; // ch? c?n d? ch?a gi� tr?

    // --- Ghi de gia tri nhiet do ---
    CLCD_I2C_SetCursor(&LCD1, 6, 0); // v? tr� sau "Temp: "
    sprintf(buf, "%.1f", temp);
    CLCD_I2C_WriteString(&LCD1, "     "); // x�a gi� tr? cu (5 kho?ng tr?ng)
    CLCD_I2C_SetCursor(&LCD1, 6, 0);
    CLCD_I2C_WriteString(&LCD1, buf);

    // --- Ghi d� gi� tr? d? ?m ---
    CLCD_I2C_SetCursor(&LCD1, 6, 1); // v? tr� sau "Humi: "
    sprintf(buf, "%.1f", humi);
